...
export async function getDebate(id) {
  ...
  if (!doc.exists) throw new Error("Debate not found");

  const base = { id: doc.id, ...doc.data() };
  const [rounds, roundScores, finalScore] = await Promise.all([
    fetchRounds(id),
    fetchRoundScores(id),
    fetchFinalScore(id),
  ]);
...
export async function addTranscriptSegments(id, segments) {
  await getDebate(id); // ensure debate exists
...
export async function closeRound(debateId, roundId, scoreResult) {
  ...
  if (round.debateId !== debateId) throw new Error("Round not found");
  if (round.status === "closed") throw new Error("Round already closed");
