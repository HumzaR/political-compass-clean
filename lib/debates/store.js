...
function assertCanCloseRound(status) {
  if (status !== "live" && status !== "ended") {
    throw new Error("Rounds can only be closed while debate is live or ended");
  }
}
...
export async function createDebate({
  ...
  for (let i = 0; i < roundCount; i += 1) {
    ...
    const subtopic = String(roundSubtopics[i] || "").trim();
...
export async function listDebates(maxItems = 50) {
  const limitItems = normalizeLimit(maxItems, {
    min: 1,
    max: 100,
    fallback: 50,
  });
...
export async function joinDebate(debateId, userUid, displayName = "Guest") {
  ...
  if (!existing?.joinedAtServer) {
    payload.joinedAtServer = FieldValue.serverTimestamp();
  }

  await participantRef(debateId, userUid).set(payload, { merge: true });
...
export async function addTranscriptSegments(id, segments) {
  ...
  const normalized = segments.map((s) => {
    ...
    batch.set(
      adminDb.collection(COLLECTIONS.transcriptSegments).doc(segmentId),
      item
    );
