// scripts/backfillResults.js
// Recompute axis scores from a user's answers and backfill into /results + /profiles.
//
// Sources it will try (in order):
// 1) /answers/{uid}.answers
// 2) /results/{profile.lastResultId}.answers
// 3) query /results where uid == <uid> order by createdAt desc -> answers
//
// Run: node scripts/backfillResults.js [--dry] [--limit=100] [--only=uid1,uid2]
...
// ---- Config (adjust if your collection names differ) ----
const PROFILES_COLLECTION = "profiles";
const RESULTS_COLLECTION  = "results";
const ANSWERS_COLLECTION  = "answers"; // primary place we expect answers
const SCALE = 5; // must match your lib/scoring normalization
...
// ---- Scoring (mirror of lib/scoring.js) ----
...
// ---- Robust loader for data/questions.js (works on Windows/macOS/Linux) ----
async function loadQuestions() {
  ...
  try {
    ...
    try {
      ...
      console.error("Failed to load questions via ESM and CJS:", e, e2);
      throw e2;
    }
  }
}

// ---- Try to load answers for a user from several places ----
async function loadUserAnswers(db, uid, profile) {
  ...
  if (ansDoc.exists) {
    ...
    const byId = data.answers || data; // in case the doc itself is the map
  ...
  // 3) latest /results where uid==uid
...
(async function main() {
  ...
  if (!admin.apps.length) {
    ...
    if (saPath) {
      const cred = require(path.resolve(saPath));
  ...
  for (const uid of targets) {
    try {
      ...
      // Load answers from one of the sources
      ...
      // Compute normalized axis scores
      ...
      // Update existing result or create + link
      ...
      if (existingResultId) {
        ...
        const newResult = {
          ...payload,
          answers: byId, // optional: keep a snapshot
